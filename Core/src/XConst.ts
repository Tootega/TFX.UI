class NumericInput
{
    ...
    })
    {
        ...
        this.maxIntegerDigits = options?.maxIntegerDigits ?? 4;
        this.decimalDigits = options?.decimalDigits ?? 2;
    ...
    private adjustCursorPosition(oldPos: number | null, oldValue: string, newValue: string)
    {
        ...
        {
            const decimalCursor = Math.min(
                oldPos - oldValue.indexOf(',') - 1 + commaIndex + 1,
                newValue.length
            );
